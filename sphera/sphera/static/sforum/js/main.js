...
//                    balloonContentBody: '<span class="description">' + json[i].__cat.name + '</span>',
                ...
                iconImageOffset: [-15, -40], // Смещение иконки
                iconContentOffset: [0, -12], // Смещение названия (4px выше иконки)
                iconContentLayout: ymaps.templateLayoutFactory.createClass(
                    '<div style="font-size: 12px; font-weight: bold; text-align: center; color: black;">' +
                    json[i].title + '</div>'
                ),
                iconShape: {
                    type: 'Rectangle',
                    coordinates: [[-15, -40], [15, 0]]
                }
            }
        );
        ...
        for (i = 0; i < json.length; i++) {
            var myPlacemark = new ymaps.Placemark(
                [json[i].latitude, json[i].longitude], {
                    ...
                    // Используем локально сохраненную иконку пламен
                    iconLayout: 'default#image',
                    iconImageHref: 'https://img.icons8.com/fluency/48/wow-button.png', // Путь к скаченной иконке (предположим, она лежит в папке images)
