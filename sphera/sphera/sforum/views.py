...
def main_page(request):
    ...
    banners = BannerPhoto.objects.all()[:3]
    banners_video = BannerVideo.objects.all()[:1]
    reviews = Reviews.objects.all()[:2]
    events = Events.objects.all()[:8]
    posts=CompanyPost.objects.all()[:8]
    news=CompanyNews.objects.all()[:8]
    comments = Comments.objects.all()[:10]
    news_comments = NewsComments.objects.all()[:10]
    # cats = Categories.objects.all()
    return render(request, 'sforum/index.html', {'posts':posts, 'menu': user_menu, 'news':news, 'events':events,'reviews':reviews, 'comments':comments, "banners_photo":banners,"news_comments":news_comments,"banners_video":banners_video,})
...
class CreateProfile(DataMixin, CreateView):
    ...
    fields = (
    'bio', 'profile_pic', "profile_pic_1", "profile_pic_2", "profile_pic_3", "tg_url", "vk_url", "web_site_url",
    "instagram_url",)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if str(user_group) == "Компания":
                ...
                send_mail(
                    "Subject here",
                    f"Компания  {user.email} оставила заявку на регистрацию, чтобы выдать доступ компании перейдитепо ссылке http://sferamsk.fun/swdjadmin/sforum/user/{user.id}/change/ и поставьте галочку в поле <активный>",
                    "VsferaV@yandex.ru",
                    ["VsferaV@yandex.ru"],
                    fail_silently=False,
                )
