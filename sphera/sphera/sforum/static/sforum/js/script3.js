...
const slideContainerCarousel = document.querySelector('.slides');
const prevButtonCarousel = document.querySelector('.prev_button');
const nextButtonCarousel = document.querySelector('.next_button');

function showSlideCarousel(index) {
  slideContainerCarousel.style.transform = `translateX(-${index * 100}%)`;
}

function changeSlideCarousel(direction) {
  currentSlideCarousel = (currentSlideCarousel + direction + totalSlidesCarousel) % totalSlidesCarousel;
  showSlideCarousel(currentSlideCarousel);
}

prevButtonCarousel.addEventListener('click', () => changeSlideCarousel(-1));
nextButtonCarousel.addEventListener('click', () => changeSlideCarousel(1));

setInterval(() => {
  changeSlideCarousel(1);
}, 7000);
